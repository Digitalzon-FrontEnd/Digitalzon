...
function SurveyRegist ({ modalClose, posts, setPosts }) {
    ...
    const option1Remove = (e) => {
        ...
        let newOptions = [...options1].filter((value) => {return value !== text});
        setOptions1(newOptions);
    };
    ...
    const option2Remove = (e) => {
        const text = e.target.previousSibling.innerText;
        let newOptions = [...options2].filter((value) => {return value !== text});
        setOptions2(newOptions);
    };
    ...
    const option3Remove = (e) => {
        const text = e.target.previousSibling.innerText;
        let newOptions = [...options3].filter((value) => {return value !== text});
        setOptions3(newOptions);
    };
    ...
    return(
        <div className="surveyRg">
            ...
            <form className="svRg-form-box" action="/" onSubmit={addNewPost}>
                <ul className="svRg-form-lists">
                    ...
                    <li className="svRg-form-list">
                        ...
                        <input id="svRgSurveyUrl" type="url" placeholder="설문 URL을 기입해주세요" ref={link} required />
                    </li>
                    <li className="svRg-form-list">
                        <label htmlFor="svRgNeededSamples">필요샘플 수</label>
                        <input id="svRgNeededSamples" type="number" min={0} placeholder="숫자만 입력가능합니다." ref={needSample} required />
                        <label htmlFor="svRgPointPerPerson" className="svRg-label-point">참여 포인트</label>
                        <input id="svRgPointPerPerson" type="number" min={0} placeholder="숫자만 입력가능합니다." ref={pointPerPerson} required />
                    </li>
                    <li className="svRg-form-list">
                        <label htmlFor="svRgFileName">방송패널 등록</label>
                        <input id="svRgFileRegister" type="file" ref={fileInput} onChange={movefileName} required />
                        ...
                        <ul className="svRg-profile-lists">
                            <li className="svRg-profile-list">
                                ...
                                <div className="svRg-options-box">
                                    <select className="svRg-options-depth1" onChange={e => setProfile1(e.target.value)}>
                                        <option value="">항목선택</option>
                                        <option value="sex">성별</option>
                                        <option value="age">나이</option>
                                    </select>
                                    {profile1 === "sex" ?
                                    <select className="svRg-options-depth2" onChange={addOption1btn}>
                                        ...
                                        {options1.map((item,index) => {
                                            return(
                                                <div className="svRg-optionbtn" key={index}>
                                                    <span>{item}</span>
                                                    <span className="svRg-btn-optionRemove" onClick={option1Remove}>x</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                            ...
                            <li className="svRg-profile-list">
                                ...
                                <div className="svRg-options-box">
                                    <select className="svRg-options-depth1" onChange={e => setProfile2(e.target.value)}>
                                        <option value="">항목선택</option>
                                        <option value="mobileOpreator">통신사</option>
                                    </select>
                                    {profile2 === "mobileOpreator" ?
                                    <select className="svRg-options-depth2" onChange={addOption2btn}>
                                    ...
                                    <div className="svRg-optionbtn-box">
                                        {options2.map((item,index) => {
                                            return(
                                                <div className="svRg-optionbtn" key={index}>
                                                    <span>{item}</span>
                                                    <span className="svRg-btn-optionRemove" onClick={option2Remove}>x</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                            ...
                            <li className="svRg-profile-list">
                                ...
                                <div className="svRg-options-box">
                                    <select className="svRg-options-depth1" onChange={e => setProfile3(e.target.value)}>
                                        <option value="">항목선택</option>
                                        <option value="electricDevice">전자기기</option>
                                    </select>
                                    { profile3 === "electricDevice" ?
                                    <select className="svRg-options-depth2" onChange={e => setProfile3_2(e.target.value)}>
                                        <option value="">전자기기 선택</option>
                                        <option value="cellphone">휴대폰</option>
                                        <option value="notebook">노트북</option>
                                    </select> : null }
                                    { profile3_2 === "cellphone" ?
                                    <select className="svRg-options-depth3" onChange={addOption3btn}>
                                    ...
                                    <div className="svRg-optionbtn-box">
                                        {options3.map((item,index) => {
                                            return(
                                                <div className="svRg-optionbtn" key={index}>
                                                    <span>{item}</span>
                                                    <span className="svRg-btn-optionRemove" onClick={option3Remove}>x</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                    ...
                    <li className="svRg-form-list" id="svRgSubmitBtnList">
                        <button className="svRg-btn-submit btn-s btn-o" type="submit" onClick={setNewPost}>저장</button>
                    </li>
                </ul>
            </form>
        </div>
